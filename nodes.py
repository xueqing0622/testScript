...
class SaveImage:
    ...
    def save_images(self, images, filename_prefix="ComfyUI", prompt=None, extra_pnginfo=None):
        ...
        try:
            counter = max(
                filter(
                    ...
                    map(map_filename, os.listdir(self.output_dir)),
                )
            )[0] + 1
        except ValueError:
            counter = 1
        ...
        for file in os.listdir(self.output_dir):
            if file.startswith('new_text') and file.endswith('.txt') and not file.endswith('.jpg.txt'):  
                text_file_list.append(file)

        new_text_file_name = f"{filename_prefix}_{counter:05}_.txt"
        with open(os.path.join(self.output_dir, new_text_file_name), 'w') as outfile:
            for cnt, text_file_name in enumerate(text_file_list):
                with open(os.path.join(self.output_dir, text_file_name), 'r') as infile:
                    outfile.write(infile.read())
                if cnt != len(text_file_list)-1:
                    outfile.write('~~~~~~')

        for image in images:
            i = 255.0 * image.cpu().numpy()
            img = Image.fromarray(np.clip(i, 0, 255).astype(np.uint8))
            file = f"{filename_prefix}_{counter:05}_.jpg"
            img.save(
                os.path.join(self.output_dir, file), jpegquality=96, optimize=True
            )
            counter += 1

        for file in os.listdir(self.output_dir):
            if file.startswith('new_text') and file.endswith('.txt'):
